  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      options: {
        data: { 
          name, 
          role: 'customer' // Garante que é um cliente comum
        },
      },
    });
      ...
      <div className="flex-1 px-6 -mt-10 relative z-20 rounded-t-[32px] pt-10 shadow-2xl bg-white">
        <div className="max-w-md mx-auto">
          <div className="mb-6">
            ...
            <p className="text-slate-500 mt-1 text-sm text-balance">Receba atualizações dos seus pedidos em tempo real</p>
          </div>
          ...
          <div className="text-center mt-6 pb-12">
            <p className="text-slate-500 text-sm">
              Já tem conta? <Link href="/login" className="font-bold ml-1" style={{ color: P }}>Fazer login</Link>
            </p>
          </div>
        </div>
      </div>
