...
export default function RegisterUserPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: '#f8f6f6' }}>
        <div className="bg-white rounded-3xl p-10 text-center max-w-sm w-full shadow-sm border border-slate-100">
          ...
          <h2 className="text-xl font-bold text-slate-900 mb-2">Quase lá!</h2>
          <p className="text-slate-500 text-sm">
            Enviamos um link para confirmar seu email em <strong className="text-slate-700">{email}</strong>.
          </p>
          <Link href="/login-user" className="block mt-6 font-semibold text-sm" style={{ color: P }}>Ir para o login →</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden" style={{ backgroundColor: '#f8f6f6' }}>
      ...
      <div className="flex-1 px-6 -mt-10 relative z-20 rounded-t-[32px] pt-10 shadow-2xl bg-white">
        <div className="max-w-md mx-auto">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-slate-900">Criar sua conta</h2>
            <p className="text-slate-500 mt-1 text-sm">Receba atualizações dos seus pedidos em tempo real</p>
          </div>

          {error && (
            <div className="bg-red-50 border border-red-200 text-red-700 text-xs px-4 py-3 rounded-xl mb-4">{error}</div>
          )}
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            {/* Celular */}
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-slate-700 ml-1">Celular / WhatsApp</label>
              <input
            ...
            {/* CPF */}
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-semibold text-slate-700 ml-1">CPF <span className="text-slate-400 font-normal">(para pagamentos)</span></label>
              <input
          ...
          <div className="text-center mt-6 pb-12">
            <p className="text-slate-500 text-sm">
              Já tem conta? <Link href="/login-user" className="font-bold ml-1" style={{ color: P }}>Fazer login</Link>
            </p>
          </div>
        </div>
      </div>
