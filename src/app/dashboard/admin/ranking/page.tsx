...
import type { RankingSetting, VendorSubscription } from '@/types/database';
...
const SUB_FEATURE_LABELS: Record<string, { label: string; desc: string }> = {
  hide_ranking:    { label: 'Ocultar do Ranking',  desc: 'Vendor fica invisível no ranking público' },
  featured_badge:  { label: 'Badge Destaque',      desc: 'Selo de "Destaque" na listagem de quiosques' },
  featured_dishes: { label: 'Pratos em Destaque',  desc: 'Pratos aparecem primeiro na busca' },
};
...
export default function RankingPage() {
  ...
  const [newSub, setNewSub] = useState({ vendor_id: '', feature: 'featured_badge', price_paid: '', expires_days: '30' });
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/30"
                >
                  {Object.entries(SUB_FEATURE_LABELS).map(([key, val]) => (
        ...
        <section>
          ...
          <div className="grid gap-3">
            {subs.length === 0 ? (
              ...
              subs.map(sub => {
                const info = SUB_FEATURE_LABELS[sub.feature] ?? { label: sub.feature, desc: '' };
