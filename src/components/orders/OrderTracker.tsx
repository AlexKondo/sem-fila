...
export default function OrderTracker({ initialOrder }: { initialOrder: any }) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors" style={{ fontFamily: "'Public Sans', sans-serif" }}>
      {/* Payment result modal */}
      {showPaymentModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 w-full max-w-sm text-center shadow-2xl border dark:border-slate-800">
            ...
            {paymentStatusResult === 'success' ? (
              <>
                ...
                <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-1">Pagamento confirmado!</h2>
                <p className="text-slate-500 dark:text-slate-400 text-sm mb-5">Seu pedido foi pago e está sendo preparado.</p>
              </>
            ) : (
              <>
                ...
                <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-1">Pagamento cancelado</h2>
                <p className="text-slate-500 dark:text-slate-400 text-sm mb-5">Você pode tentar pagar novamente abaixo.</p>
              </>
            )}
            <button
      ...
      {/* Header */}
      <div className="bg-white dark:bg-slate-900 border-b border-slate-100 dark:border-slate-800 px-4 py-4 sticky top-0 z-40">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div>
            <p className="text-xs text-slate-400 dark:text-slate-500 mb-0.5">{order.vendors?.name}</p>
            <h1 className="text-lg font-bold text-slate-900 dark:text-white">Pedido #{order.pickup_code}</h1>
            {order.table_number && (
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <button onClick={shareOrder} className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
              </svg>
            </button>
            <Link href="/profile/orders" className="w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-3">
        ...
        {!isCancelled ? (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 shadow-sm">
            <h2 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-4">Acompanhe seu pedido</h2>
              ...
              {STATUS_STEPS.filter(s => s !== 'cancelled').map((step, idx) => {
                ...
                return (
                  <div key={step} className="flex items-center gap-3">
                    <div
                      className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 transition-all shadow-sm ${!isCompleted && !isCurrent ? "bg-slate-100 dark:bg-slate-800" : ""}`}
                      style={{ backgroundColor: isCompleted ? '#22c55e' : isCurrent ? PRIMARY : undefined }}
                    >
                      {isCompleted ? (
          ...
          <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 rounded-2xl p-5 text-center">
        ...
        {/* Order summary */}
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 shadow-sm">
          <h2 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Resumo do pedido</h2>
          <div className="space-y-2.5">
            {order.order_items?.map((item: any) => (
              <div key={item.id} className="pt-2 border-b border-slate-50 pb-2 last:border-0">
                ...
                {item.extras && item.extras.length > 0 && (() => {
                  ...
                  return (
                    <div className="flex flex-col gap-0.5 mt-1 ml-4 pl-2 border-l-2 border-slate-100 dark:border-slate-800">
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-3 mt-1 flex items-center justify-between">
              <span className="font-bold text-slate-900 dark:text-white text-base">Total Final</span>
        ...
        {getRealNotes(order.notes) && (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-4 shadow-sm">
            <p className="text-xs text-slate-400 dark:text-slate-500 font-medium mb-1">Observações</p>
            <p className="text-sm text-slate-700 dark:text-slate-300">{getRealNotes(order.notes)}</p>
          </div>
        )}
        ...
        <div className="pt-4 pb-12 flex flex-col gap-3">
          <Link 
            href="/"
            className="w-full py-4 rounded-2xl text-center font-black text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 hover:border-orange-500 hover:text-orange-500 transition-colors shadow-sm"
          >
            FAZER NOVO PEDIDO
          </Link>
