...
import { Send, Users, LayoutGrid, DollarSign, Plus, Trash2, Shuffle, Minus, Square, Map } from 'lucide-react';
import type { CanvasLayout } from '@/components/org/EventCanvasEditor';

const EventCanvasEditor = lazy(() => import('@/components/org/EventCanvasEditor'));
...
type Tab = 'invitations' | 'layout' | 'map' | 'revenue' | 'configs';
...
export default function EventHubClient({
  ...
  initialCanvasLayouts?: CanvasLayout[];
    ...
    <div className="max-w-3xl mx-auto px-4 py-4 space-y-4 transition-colors duration-300">
      <div className="flex bg-white dark:bg-slate-900 rounded-2xl shadow-sm p-1 gap-1 border border-slate-100 dark:border-slate-800 transition-colors">
        {[
          ...
          { key: 'map' as Tab, label: 'Mapa', icon: <Map className="w-4 h-4" /> },
      ...
      {tab === 'map' && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-4">
          <Suspense fallback={<div className="flex items-center justify-center h-64 text-slate-400 text-sm">Carregando editor de mapa…</div>}>
            <EventCanvasEditor eventId={event.id} initialLayouts={initialCanvasLayouts} availableVendors={availableVendors} />
          </Suspense>
        </div>
      )}
