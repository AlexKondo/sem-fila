    ...
    <section className="mb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {vendors.map(v => (
          <div
            key={v.id}
            className={`p-4 rounded-2xl border transition shadow-sm ${v.id === currentVendorId ? 'bg-orange-600 border-orange-500 shadow-orange-300/50' : 'bg-white border-slate-100 hover:border-orange-200'}`}
          >
            ...
            <h3 className={`font-bold text-sm ${v.id === currentVendorId ? 'text-white' : 'text-slate-800'}`}>{v.name}</h3>
            <p className={`text-[11px] mb-3 ${v.id === currentVendorId ? 'text-orange-100/70' : 'text-slate-400 font-medium'}`}>{v.description || 'Nenhuma descrição'}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-xl border border-slate-100">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-bold text-slate-900">Nova Marca</h3>
              <button onClick={() => { setShowModal(false); setError(''); }} className="text-slate-400 hover:text-slate-600 text-xl leading-none">&times;</button>
            </div>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1 ml-1 uppercase tracking-widest">Nome da Marca *</label>
                <input
                  type="text"
                  required
                  value={brandName}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1 ml-1 uppercase tracking-widest">Descrição <span className="text-slate-300 font-normal normal-case">(opcional)</span></label>
                <input
                  type="text"
                  value={description}
